    ...
    public abstract class Formatter<T> : IFormatter
    {
        ...
        static Formatter()
        {
            ...
            try
            {
                ...
                else if (t.GetTypeInfo().IsGenericType && t.GetGenericTypeDefinition() == typeof(IList<>))
                {
                    var formatterType = typeof(ListFormatter<>).MakeGenericType(t.GetTypeInfo().GetGenericArguments());
                    formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                }

                else if (t.GetTypeInfo().IsGenericType && t.GetGenericTypeDefinition() == typeof(IReadOnlyList<>))
                {
                    var formatterType = typeof(ReadOnlyListFormatter<>).MakeGenericType(t.GetTypeInfo().GetGenericArguments());
                    formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                }
                ...
                else if (t.GetTypeInfo().IsGenericType && t.GetGenericTypeDefinition() == typeof(Nullable<>) && t.GetTypeInfo().GetGenericArguments()[0].GetTypeInfo().IsEnum)
                {
                    var underlyingType = Enum.GetUnderlyingType(t.GetTypeInfo().GetGenericArguments()[0]);
                    switch (Type.GetTypeCode(underlyingType))
                    {
                        case TypeCode.SByte:
                            {
                                var formatterType = typeof(NullableSByteEnumFormatter<>).MakeGenericType(t.GetTypeInfo().GetGenericArguments()[0]);
                                formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                            }
                            break;
                        case TypeCode.Byte:
                            {
                                var formatterType = typeof(NullableByteEnumFormatter<>).MakeGenericType(t.GetTypeInfo().GetGenericArguments()[0]);
                                formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                            }
                            break;
                        case TypeCode.Int16:
                            {
                                var formatterType = typeof(NullableInt16EnumFormatter<>).MakeGenericType(t.GetTypeInfo().GetGenericArguments()[0]);
                                formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                            }
                            break;
                        case TypeCode.UInt16:
                            {
                                var formatterType = typeof(NullableUInt16EnumFormatter<>).MakeGenericType(t.GetTypeInfo().GetGenericArguments()[0]);
                                formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                            }
                            break;
                        case TypeCode.Int32:
                            {
                                var formatterType = typeof(NullableInt32EnumFormatter<>).MakeGenericType(t.GetTypeInfo().GetGenericArguments()[0]);
                                formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                            }
                            break;
                        case TypeCode.UInt32:
                            {
                                var formatterType = typeof(NullableUInt32EnumFormatter<>).MakeGenericType(t.GetTypeInfo().GetGenericArguments()[0]);
                                formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                            }
                            break;
                        case TypeCode.Int64:
                            {
                                var formatterType = typeof(NullableInt64EnumFormatter<>).MakeGenericType(t.GetTypeInfo().GetGenericArguments()[0]);
                                formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                            }
                            break;
                        case TypeCode.UInt64:
                            {
                                var formatterType = typeof(NullableUInt64EnumFormatter<>).MakeGenericType(t.GetTypeInfo().GetGenericArguments()[0]);
                                formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                            }
                            break;
                        default:
                ...
                else if (t.GetTypeInfo().IsGenericType)
                {
                    ...
                    {
                        var formatterType = typeof(DictionaryFormatter<,>).MakeGenericType(t.GetTypeInfo().GetGenericArguments());
                        formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                    }
                    else if (t.GetGenericTypeDefinition() == typeof(IReadOnlyDictionary<,>))
                    {
                        var formatterType = typeof(ReadOnlyDictionaryFormatter<,>).MakeGenericType(t.GetTypeInfo().GetGenericArguments());
                        formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                    }

                    else if (t.GetGenericTypeDefinition() == typeof(DictionaryEntry<,>))
                    {
                        var formatterType = typeof(DictionaryEntryFormatter<,>).MakeGenericType(t.GetTypeInfo().GetGenericArguments());
                        formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                    }

                    else if (t.GetGenericTypeDefinition() == typeof(ILookup<,>))
                    {
                        var formatterType = typeof(LookupFormatter<,>).MakeGenericType(t.GetTypeInfo().GetGenericArguments());
                        formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                    ...
                    {
                        var formatterType = typeof(GroupingSegmentFormatter<,>).MakeGenericType(t.GetTypeInfo().GetGenericArguments());
                        formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                    }
                    ...
                    else if (t.GetTypeInfo().GetInterfaces().Any(x => x == typeof(IKeyTuple)))
                    {
                        ...
                        switch (t.GetTypeInfo().GetGenericArguments().Length)
                        {
                            case 1:
                        ...
                        var formatterType = tupleFormatterType.MakeGenericType(t.GetTypeInfo().GetGenericArguments());
                        formatter = (Formatter<T>)Activator.CreateInstance(formatterType);
                    }
