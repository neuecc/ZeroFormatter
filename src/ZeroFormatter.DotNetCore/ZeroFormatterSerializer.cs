    ...
    public static class ZeroFormatterSerializer
    {
        ...
        public static void Serialize<T>(T obj, Stream stream)
        {
            ...
            if (ms != null && ms.Position == 0)
            {
                ...
                if (zeroFormatterObj != null && zeroFormatterObj.CanDirectCopy())
                {
                    ...
                    ArraySegment<byte> dest;
                    if (ms.TryGetBuffer(out dest))
                        Buffer.BlockCopy(bufferRef.Array, bufferRef.Offset, dest.Array, 0, bufferRef.Count);
                }
            }
            else
            {
        ...
        public static T Deserialize<T>(Stream stream)
        {
            ...
            if (ms != null)
            {
                ...
                if (ms.TryGetBuffer(out arraySegment))
                {
                    var formatter = Formatter<T>.Default;
                    int _;
