...
namespace ZeroFormatter.Analyzer
{
    public static class ConfigurationLoader
    {
        ...
        {
            var config = option.AdditionalFiles.FirstOrDefault(x => System.IO.Path.GetFileName(x.Path).Equals("ZeroFormatterAnalyzer.json", StringComparison.OrdinalIgnoreCase));
            if (config != null)
            {
                try
                {
                ...
                {
                    System.Diagnostics.Debug.WriteLine("Can't load ZeroFormatterAnalyzer.json:" + ex.ToString());
                    return Empty;
                }
            }
            else
